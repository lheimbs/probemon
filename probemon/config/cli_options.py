...
def cli_options(main: FunctionType) -> FunctionType:
    ...
    main = click.option('--config', '-c', type=click.Path(), help="Provide a config file.")(main)
    main = click.option(
        '--lower', '-l', is_flag=True, help="Convert mac and venodor strings to lowercase."
    )(main)
    main = click.option(
        '--raw', '-r', is_flag=True, help="Include hex formatted raw probe request packet."
    )(main)
    main = click.option(
        '--vendor', '-v', is_flag=True, help="Try to get vendor correlating to caputred mac address."
    )(main)
    main = click.option(
        '--worker-threads',
        '-t',
    ...
    main = click.option(
        ...
        default=0,
        help="Number probe requests to capture before exiting. 0 for infinite probes. Default 0.",
    )(main)
    main = click.option('--debug/--no-debug', default=False, help='Debug flag')(main)
    main = click.option(
        '--no-stdout', is_flag=True, help="Disable printing probes to stdout. DOES NOT DISABLE LOGGING!"
    )(main)
    ...
    main = channel_group.option('--channel-set', type=int, help="Manually set wifi channel.")(main)
    main = channel_group.option('--channel-auto', is_flag=True, help="Automatically set wifi channel.")(main)
    ...
    main = mqtt_group.option('--mqtt-host', help='Broker host name')(main)
    main = mqtt_group.option('--mqtt-port', type=int, default=1883, help='Broker port')(main)
    main = mqtt_group.option('--mqtt-topic', help="Topic to publish probes under.")(main)
    main = mqtt_group.option('--mqtt-user', help='Mqtt username')(main)
    main = mqtt_group.option('--mqtt-password', help='Password for mqtt user')(main)
    ...
    main = mqtt_group.option('--mqtt-debug', is_flag=True, help='Set mqtt client debugging individually.')(main)
    ...
    main = sql_group.option('--sql-host', help='Sql host name')(main)
    main = sql_group.option('--sql-port', type=int)(main)
    main = sql_group.option('--sql-user', help='Username to connect to the database with')(main)
